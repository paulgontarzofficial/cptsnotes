...
```shell-session
realCustampin@htb[/htb]$ curl -s http://drupal.inlanefreight.local | grep Drupal
...
![Inlanefreight Blog page with a 'Coming soon' message for a new employee blog and industry news site.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/113/drupal_node.png)
...
```shell-session
realCustampin@htb[/htb]$ curl -s http://drupal-acc.inlanefreight.local/CHANGELOG.txt | grep -m2 ""
...
```shell-session
realCustampin@htb[/htb]$ curl -s http://drupal.inlanefreight.local/CHANGELOG.txt
...
```shell-session
realCustampin@htb[/htb]$ droopescan scan drupal -u http://drupal.inlanefreight.local
