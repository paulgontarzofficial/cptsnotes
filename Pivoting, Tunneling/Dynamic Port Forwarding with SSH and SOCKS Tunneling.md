...
![[Pasted image 20251104162916.png]]
- We have a target host (10.129.15.50) which we can scan using nmap: 

```shell-session
realCustampin@htb[/htb]$ nmap -sT -p22,3306 10.129.202.64
...
- We have one of two options here, we can access the mysql using ssh from the ubuntu server or we can forward the traffic back to our localhost on port 1234: 

```shell-session
realCustampin@htb[/htb]$ ssh -L 1234:localhost:3306 ubuntu@10.129.202.64
...
- We should now be able to access the MySQL service locally on port 1234. 

We can confirm that we have the port forward set up using netstat or nmap against our localhost: 
```shell-session
realCustampin@htb[/htb]$ netstat -antp | grep 1234
...
```shell-session
realCustampin@htb[/htb]$ nmap -v -sV -p1234 localhost
...
- Let's say we want to forward multiple ports from the Ubuntu server to your localhost, you can do so by including the local port:server:port argument to your ssh command: 
```shell-session
realCustampin@htb[/htb]$ ssh -L 1234:localhost:3306 -L 8080:localhost:80 ubuntu@10.129.202.64
```

### Setting up Pivot 

- If you type ifconfig or ip a on the ubuntu host, you will find that this server has multiple NICs: 
	- One connected to our attack host
	- One communicating to other hosts within the same network
...
lo: flags=73<UP,LOOPBACK,RUNNING>  mtu 65536
        ...
        TX errors 0  dropped 0 overruns 0  carrier 0  collisions 0
```
- Now that we have all the NICs setup, we know want the ability to scan the internal network (172.16.x.x) from our attack host. In order for us to this though, we need to set up dynamic port forwarding to pivot our network packets via the Ubuntu server. 
- We can do this by setting up a SOCKS listener on our local host and then configure SSH to forward that traffic via SSH to the internal network. 

SOCKS = Socket Secure

![[Pasted image 20251104164349.png]]

**Enabling Dynamic Port Forwarding with SSH**
```shell-session
realCustampin@htb[/htb]$ ssh -D 9050 ubuntu@10.129.202.64
```
- The -D argument requests the SSH server to enable dynamic port forwarding. Once enabled, we require a tool that can route any tool's paclets over the port 9050. 
- Proxychains is capable of getting tool packets over port 9050. 

Proxychains Configuration File: 
```shell-session
realCustampin@htb[/htb]$ tail -4 /etc/proxychains.conf

# meanwile
# defaults set to "tor"
socks4 	127.0.0.1 9050
```
- Currently, we have a SOCKS4 proxy set up on the localhost over port 9050. 

```shell-session
realCustampin@htb[/htb]$ proxychains nmap -v -sn 172.16.5.1-200
...
<SNIP>
```
- Notice that running a tool through the proxy requires you to pass it through proxychains first then run the tool. 
- This process is called SOCKS Tunneling 

Using nmap with proxychains to enumerate windows host machines: 
```shell-session
realCustampin@htb[/htb]$ proxychains nmap -v -Pn -sT 172.16.5.19
...
**Using Proxychains to Pivot msfconsole**

```shell-session
realCustampin@htb[/htb]$ proxychains msfconsole
...
**Finishing Touch**
- Now that we have a windows machine that can be connected to via RDP, let's use proxychains with xfreerdp to login: 
...
**Solution:**
1. Log in to pivot host (ubuntu) using the 'ssh -D 9050 10.129.202.64', with credentials that were given. 
2. Once you have the proxy set, run xfreerdp with proxychains. 
3. Log in to Windows Machine with creds that were given and view the flag on desktop. 
	- N1c3Piv0t