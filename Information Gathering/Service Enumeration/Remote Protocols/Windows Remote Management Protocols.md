- Enabled by default starting with Windows Server 2016
...
- By default, RDP operates over TCP/3389
- Activated using the Server Manager Application 
- Allows connections to the service only to hosts with Network Level Authentication (NLA)
...
```
realCustampin@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --script rdp*
...
```
realCustampin@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --packet-trace --disable-arp-ping -n
...
```
realCustampin@htb[/htb]$ sudo cpan
...
**RDP Security Check:**
- This security check was developed by Cisco CX Security and it's intention was to obtain RDP Settings without being an authenticated user using the handshakes. 


```
realCustampin@htb[/htb]$ git clone https://github.com/CiscoCXSecurity/rdp-sec-check.git && cd rdp-sec-check
realCustampin@htb[/htb]$ ./rdp-sec-check.pl 10.129.201.248
...
## Using Linux to Access Windows Server via RDP
...
```
realCustampin@htb[/htb]$ xfreerdp /u:cry0l1t3 /p:"P455w0rd!" /v:10.129.201.248
...
- Utilizes the Simple Object Access Protocol (SOAP)
- Relies on TCP/5985 (HTTP),5986 (HTTPS)

## Footprinting WinRM

Using nmap: 
```
realCustampin@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p5985,5986 --disable-arp-ping -n
...
**Using evil-winrm to interact with WinRM:**

```
realCustampin@htb[/htb]$ evil-winrm -i 10.129.201.248 -u Cry0l1t3 -p P455w0rD!
...
- Accessed via PowerShell, VBScript, or the Windows Management Instrumentation Console (WMIC)

## Footprinting Windows Management Instrumentation) 

- WMI Communication happens over Port TCP/135

Tools: 
- **WMIexec.py**

```
realCustampin@htb[/htb]$ /usr/share/doc/python3-impacket/examples/wmiexec.py Cry0l1t3:"P455w0rD!"@10.129.201.248 "hostname"
