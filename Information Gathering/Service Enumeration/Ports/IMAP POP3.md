...
Internet Message Access Protocol (143 and 993)

Post Office Protocol v3 (110 and 995)
...
```
realCustampin@htb[/htb]$ sudo nmap 10.129.14.128 -sV -p110,143,993,995 -sC
...
```
realCustampin@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user user:p4ssw0rd

* LIST (\HasNoChildren) "." Important
* LIST (\HasNoChildren) "." INBOX
```
- Using the curl command and supplying credentials, we are able to log in and view the mailbox of that user. 


```
realCustampin@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user cry0l1t3:1234 -v
...
Using OpenSSL to interact with IMAPS and POP3
```
realCustampin@htb[/htb]$ openssl s_client -connect 10.129.14.128:imaps
...
```
realCustampin@htb[/htb]$ openssl s_client -connect 10.129.14.128:pop3s
...
6. ### Try to access the emails on the IMAP server and submit the flag as the answer. (Format: HTB{...})
	1. HTB{983uzn8jmfgpd8jmof8c34n7zio}
...
a unselect DEV
a OK Unselect completed (0.001 + 0.000 secs).
a select DEV.DEPARTMENT    
a NO Mailbox doesn't exist: DEV.DEPARTMENT (0.001 + 0.000 secs).
...
a OK Fetch completed (0.004 + 0.000 + 0.003 secs).
a fetch 1 body
