- The group "Print Operators" is another very powerful group that has the ability to create, share, and delete printers that are connected to the Domain Controller. 
- The privilege that we are targetting is the SeLoadDriverPrivilege. 
...
- It is well known that the `capcom.sys` has the ability to allow any user to execute code with SYSTEM Privileges. We can use our privileges to load this vulnerable driver and escalate privileges. We can use [this](https://raw.githubusercontent.com/3gstudent/Homework-of-C-Language/master/EnableSeLoadDriverPrivilege.cpp) tool to load the driver. The PoC enables the privilege as well as loads the driver for us.
...
- After we obtain the code from above, we can now compile on the target computer using Visual Studio. 
...
- Next, we can download the Capcom.sys driver from this link: https://github.com/FuzzySecurity/Capcom-Rootkit/blob/master/Driver/Capcom.sys
- We need to save it to `C:\temp` directory. 

**Adding a Reference to the Capcom.sys Driver**
```cmd-session
