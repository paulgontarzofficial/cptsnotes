- We may be able to compromise a system with an account that has the ability to install software or if a company is using insecure third-party applications. 
...
```shell-session
realCustampin@htb[/htb]$ python3 -m http.server 8080
```
...
```shell-session
realCustampin@htb[/htb]$ nc -lvnp 9443
