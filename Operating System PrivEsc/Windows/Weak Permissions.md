...
**Running SharpUp**
- This program checks for service binaries with weak ACLs.
```powershell-session
...
- We can see that the BUILTIN\Users and Everyone both have full permissions to the directory. 
```powershell-session
...
- We can copy the original binary and then pass it to msfvenom to create a malicious binary to gain a reverse shell. 
```cmd-session
...
- We can check the output again for any modifiable services. 
...
- Checking the Local Admin group, we can see that the user `htb-student` is not listed as a member. 
```cmd-session
...
- Let's try to run a command that adds the `htb-student` user to the local admin group. 
```cmd-session
...
**Confirming the Local Admin Group Addition**
