When conducting a penetration test, we may gain a low-privileged shell on a windows host and need to perform privilege escalation to further our access. We can escalate privileges  to one of the following depending on the system configuration and what type of data we encounter: 
...
Enumeration is essential to privilege escalation, for example, using the low-privileged shell access to get the lay of the land of what we are working with and gaining situational awareness. 

**Key Data Points**

OS Name: Knowing the type of Windows OS and level will give us an idea of the types of tools that may be available or lack thereof on legacy systems. 

Version: There may be public exploits when it comes to the OS version. 

Running Services: Knowing what type of services and there version could lead us to further privilege escalation. 

NT AUTHORITY\SYSTEM or an Administrator Level Account. A misconfigured or vulnerable service running in the context of a privileged account can be an easy win for privilege escalation. 

-----
### System Information
- Looking at the system will give us a better idea of exact operating system version, hardware in use, installed programs, and security updates. 

**Using the tasklist cmd to View Running Services**
```cmd-session
C:\htb> tasklist /svc
...
```
- In this example, we can take a look at the services that are currently on the system, and most notably, we have a `Filezilla` service that we can try to get the version, and look for a public exploit. 
- We can also look at the processes for Windows Defender. 
...
- The `systeminfo` command will show whether the box has been patched recently and whether or not it is a VM. Gaining admin access could be as simple as running a known POC. We can view the KBs that are installed under [HotFixes](https://www.catalog.update.microsoft.com/Search.aspx?q=hotfix) 
...
**Patches and Updates**
- If the `systeminfo` doesn't display hotfixes, they may queriable with WMI using the WMI Command binary with QFE to display patches. 
...
```
- We can also do this with PowerShell as well using the `Get-Hotfix` cmdlet. 
...
**Installed Programs**
- WMI can also be used to display installed software. This information can often guide us towards hard-to-find exploits. We can run LaZagne to check if stored credentials for those apps are installed. 
...
- The `netstat` command will display active TCP and UDP connections which will give us a better idea of what services are listening on which port(s) both locally and accessivle to the outside. 
...
- It is crucial that we get a good understanding of the Users and Groups on the system and within the organization. We may find that the system to be well patched, but a member of the local admin group's user directory is browsable and contains a password file such as a logins.xlsx, resulting in a win. 
...
**Current User**
- When we gain access to a user shell, we should always check what user context we are operating under. We may already be SYSTEM or equivalent, that would be too easy though... 
- We could have privileges such as SeImpersonatePrivilege, which can often be easily abused to escalate privileges using a tool such as Juicy Potato. 
...
**Get All Users**
- Knowing what other users are on the system is important as well. If we gained RDP access as user bob and we see that there is a bob_adm, we might want to try the same creds for re-use.
...
**Get All Groups**
- Knowing what standard and non-standard groups are present on the host can help us determine what the host is used for, how heavily accessed it is, or may even lead to discovering a misconfiguration such as all Domain Users in the Remote Desktop or local admin groups. 
...
- Though unlikely, we may gain some valuable information when we view the details of a group. 
