- There are plenty of other ways of obtaining credentials on a windows system, lets look at some of those other ways that we can obtain a password. 
...
When we attempt to use the credentials for logging into the SQL01 server, it will use the stored credentials to do so. 
...
-------
### Browser Credentials
- Retrieving saved creds from their browsers for applications that they frequently visit. 
...
- We can use the script keepass2john.py to extract the hash to be used in hashcat. 
```shell-session
realCustampin@htb[/htb]$ python2.7 keepass2john.py ILFREIGHT_Help_Desk.kdbx 
...
```shell-session
realCustampin@htb[/htb]$ hashcat -m 13400 keepass_hash /opt/useful/seclists/Passwords/Leaked-Databases/rockyou.txt
...
-------
### Email
- If we gain access to a domain-joined system in the context of a domain user with and Exchange inbox, we can use the search functionality to search for terms such as 'pass' 'password'...
...
- We can run the `all` module to run against all applicable applications that are on the system. 
...
**Viewing the Saved Wireless Network**
