- We know that we can use the account wley to kick off an attack chain that will result in us taking control of the adunn user who can perform the DCSync Attack which would give us full control of the domain. 
...
```
- After we create the PSCredential for wley, we must wcreate the SecureString Object for adunn set to the password that we want to set for user adunn. 

**Creating a SecureSting Object**
```powershell-session
PS C:\htb> $damundsenPassword = ConvertTo-SecureString 'Pwn3d_by_ACLs!' -AsPlainText -Force
```

**Changing the User adunn Password**
```powershell-session
PS C:\htb> cd C:\Tools\
...
At this point, we should be able to leverage our new group membership to take control over the adunn user. Since we now have the GenericAll rights over user adunn, we can create a fake SPN to set us up for a kerberoasting attack. 
...
```
- Now that we have obtained the hash, We can go crack the hash offline with Hashcat. Once we have the cleartext password, we could authenticate as the adunn user and perform the DCSync attack. 

**Cleanup**
1. We need to remove the fake SPN we created on the adunn user. 
2. Remove the damundsen user from the Help Desk Level 1 Group
3. Set the password for the damundsen user back to its original value or have our client set it/alert the user. 
Keep in mind the order of the cleanup procedures as access will be taken away if we arent in the Help Desk Level 1 Group anymore. 
...
```
- Now that we have removed the fake SPN (ServicePrincipalName), we can now remove the user from the group. 
...
1. Auditing for and removing dangerous ACLs
	1. Organizations should have regular AD audits performed but also train internal staff to run tools such as BloodHound and identify potentially dangerous ACLs that can be removed. 
2. Monitor Group Membership
	1. Visibility into important groups is paramount. High-impact groups in the domain should be monitored to alert IT staff of changes that could be indicative of an ACL attack chain. 
3. Audit and monitor for ACL Changes
	1. Enabling the [Advanced Security Audit Policy](https://docs.microsoft.com/en-us/archive/blogs/canitpro/step-by-step-enabling-advanced-security-audit-policy-via-ds-access) can help in detecting unwanted changes, especially [Event ID 5136: A directory service object was modified](https://docs.microsoft.com/en-us/windows/security/threat-protection/auditing/event-5136) which would indicate that the domain object was modified, which could be indicative of an ACL attack.

When looking in the Details tab, we can see the juicy info is written in Security Descripter Definition Language (SDDL) which is not human readable. 
