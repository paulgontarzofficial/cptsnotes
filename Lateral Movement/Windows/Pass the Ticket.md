- In this specific attack, we are using a stolen kerberos ticket to move laterally instead of an NTLM Hash. 

**Kerberos Refresher:**
- Authentication Ticket based system
- Kerberos does not give an account password for every service you use, rather kerberos keeps all tickets on a local machine, and presents only that service for that ticket. 
...
- We require a valid kerberos ticket in order to perform a Pass the Ticket attack: 
	- Service Ticket to allow access to a particular resource
	- Ticket Granting Ticket, which we use to request service tickets to access any resource the user hsa privileges. 

## Obtaining the Ticket
- Let's say we have obtained Local Admin rights
- Tickets are stored by LSASS and must be obtained from LSASS. As a local admin we have the ability to obtain all tickets on the system. 
...
```
- Tickets that end in '$' correspond to computer accounts. These need a ticket to interact with Active Directory
- Tickets the have a username followed by an '@' are the service accounts. 

**Using Rubeus**
- We can run Rubeous to export the tickets that are encoded with Base64. 
```cmd-session
...
- This concepts takes the rc4/ntlm hash and creates a Ticket Granting Ticket. 

Here we need to extract the Kerberos Keys in order for us to create the Ticket Granting Ticket. (TGT)
```cmd-session
c:\tools> mimikatz.exe
...
<SNIP>
```
- Now that we have access to the AES256_HMAC and RC4_HMAC, we can now create our ticket. 

This creates a new cmd session for us to create the ticket: 
```cmd-session
c:\tools> mimikatz.exe
...
Using Rubeus, we can now pass it the parameters that we gained from mimikatz and attempt to generate a new ticket: 
```cmd-session
...
## Pass the Ticket (Actual Movement)
- We can now use Rubeus to move laterally through the network, and also we use the /ptt to pass the ticket to the current session. 
```cmd-session
...
Another way to import the ticket is using the .kirbi file that we obtained from mimikatz. 
...
Now, we can use a Base64 string on Rubeus. 
```cmd-session
...
- We can also use mimikatz to attempt the pass the ticket attack using the .kirbi file that was obtained from earlier. 
```cmd-session
C:\tools> mimikatz.exe 
...
## Pass the Ticket w/ PowerShell Remoting

- To create a PowerShell Remoting session on a remote computer, you must have administrative permissions, be a member of the Remote Management Users group, or have explicit PowerShell Remoting permissions in your session configuration.
- Let's say we have a user that is part of the Remote Users Group, we can utilize this method to gain access to the Domain Controller: 
...
## PowerShell Remoting w/ Rubeus: 
- The command createonly creates a sacrificial process/logon session. 
- This session is hidden by default however if we pass the /show command, we can  see the output.
- 
```cmd-session
...
Once in the new session, we can do a ptt attack on the domain controller to try to get in there: 
```cmd-session
