...
![Employee Manager with 'Edit Profile' button, links to 'Personal Records', 'Documents', and 'Contracts'](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/134/web_attacks_idor_employee_manager.jpg)
...
![Employee Documents with links to 'Documents', 'Invoice', and 'Report'](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/134/web_attacks_idor_documents.jpg)
...
```shell-session
realCustampin@htb[/htb]$ curl -s "http://SERVER_IP:PORT/documents.php?uid=3" | grep "<li class='pure-tree_link'>"
...
```shell-session
realCustampin@htb[/htb]$ curl -s "http://SERVER_IP:PORT/documents.php?uid=3" | grep -oP "\/documents.*?.pdf"
