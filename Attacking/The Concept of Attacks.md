...
![[Pasted image 20251031181000.png]]

1. Source: We have a source that is responsible to perform a specific request to a Process. 
2. Process: A process is where the vulnerability gets triggered. 
3. Privileges: Each process has a privilege with which it is executed. 
4. Destination: And with each process has a destination or end goal in mind to either compute new data or forward it. 

## Source: 
- Source of information that is used for a specific task of a process. 
...
## Processes: 
- Processing the information from the source. 
...
## Privileges: 
- These represent the controlling authority of the processes that are being ran on the system. 
...
## Destination: 
- Every process has at least one task that needs to be fulfilled. 
...
![[Pasted image 20251031182439.png]]
